...
Pravidlo: `confidence === "Certain"` → render `variantB` (název + popis). `confidence === "Likely"` → render `variantA` (jen názvy, bez popisu). Toto rozlišení je záměrné — kratší obsah u Likely štítků nemá být kompenzován vycpáváním textu, ale řešen vizuální hierarchií (větší font, více white space).
